...
코드의 논리적 구획을 나누기 위해 `--#region` 과 `--#endregion` 을 사용합니다. 이는 IDE 에서 접기/펼치기 기능을 지원하여 긴 파일의 내비게이션을 돕습니다.

- **권장 구획**: `Constructors`, `Methods`, `Events`, `Internal Functions` 등.
- 단순한 변수 선언이나 짧은 코드는 굳이 Region 으로 감싸지 않습니다.
