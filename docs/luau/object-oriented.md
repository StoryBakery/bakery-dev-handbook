---
title: Luau Object Oriented Programming Style
---

Story-Bakery 에서 사용하는 객체지향(OOP) 패턴과 모듈 내부 구조에 대한 가이드입니다.
...
`_` 로 시작하는 프라이빗 메서드(`_helper`)는 이를 호출하는 **공개 메서드 위**에 배치합니다.
호출 관계가 눈에 보이도록 묶어두는 것이 핵심입니다.

```lua
-- 모듈 내에서만 사용하는 헬퍼 메서드의 경우, 첫번째 인수가 self 인 로컬함수로:
local function _helper(self: Class, ...)
    -- ...
end

-- 함수 주석
function Class:PublicMethod(...)
    _helper(self, ...)
end

-- 모듈 바깥에서도 사용할 수 있게해야하는 헬퍼 함수의 경우, 콜론 `:` 으로 정의한 메서드로:
function Class:_helper(self: Class)
    -- ...
end
